...
namespace kubvc::math {
    ...
    Expression::~Expression() {        
        m_valid = false;
        m_varDepController.clear();
        m_tree.clear();
        m_plotBuffer.clear();
        m_plotBuffer.shrink_to_fit();
    }

    static constexpr std::int8_t NEWTON_MAX_ITER = 16; 
    ...
    void Expression::eval(const GraphLimits& limits, std::int32_t maxPointCount) {   
        ...
        taskManager->add([this, limits, maxPointCount] { 
            ...
            const auto left = m_varDepController.getPreferedLeft();
            const bool isYPrefered = left == nullptr || left->value == 'y';
